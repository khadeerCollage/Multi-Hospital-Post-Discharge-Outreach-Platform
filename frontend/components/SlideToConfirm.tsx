...
export interface SlideConfirmProps {
  ...
  width?: number;
  /* Label displayed inside the track */
  label?: string;
  /* Label displayed once confirmed */
  confirmedLabel?: string;
  /* Callback triggered on successful completion */
  onConfirm?: () => void;
  /* Visual color variant */
  variant?: 'default' | 'emerald' | 'blue' | 'crimson';
...
export function SlideConfirm({
  ...
  const finish = () => {
    ...
    // Notify caller that slide action committed
    try {
      onConfirm?.();
    } catch (err) {
  ...
  return (
    <div
      ...
      style={{ width: span, height: H, opacity: disabled ? 0.6 : 1, pointerEvents: disabled ? 'none' : 'auto' }}
    >
      <motion.div
        className="sld-track"
        ref={track}
...
export const SlideToConfirm = SlideConfirm;
